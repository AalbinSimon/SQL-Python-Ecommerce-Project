...
query = '''select upper(product_category) Category, round(sum(payment_value),2) Sales from products join order_items 
...
query = '''select * , dense_rank() over(order by revenue desc) as 'rank' from 
 (select order_items.seller_id , sum(payment_value) revenue from order_items join payments on
...
df = df.head()
#This part is for plotting a graph
sns.barplot(x='Seller_id',y='Revenue',data = df)
plt.xticks(rotation = 90) #Roation of label on x-axis
plt.show()
...
df = df.head() # It will show for top 5
sns.barplot(x='Customer_id',y='mov_avg',data = df)
plt.xticks(rotation = 90)
plt.title('3-Order Moving Average of Payments per Customer')
...
payments.order_id = orders.order_id  group by years , months
...
query = '''with a as
(select customers.customer_id , min(orders.order_purchase_timestamp) first_order from customers join orders
on customers.customer_id = orders.customer_id group by customers.customer_id),
b as
(select a.customer_id , count(distinct orders.order_purchase_timestamp) next_order from a join
orders on orders.customer_id = a.customer_id 
and orders.order_purchase_timestamp > first_order
 and orders.order_purchase_timestamp < date_add(first_order, interval 6 month)
group by a.customer_id)
select 100* (count(distinct b.customer_id)/count(distinct a.customer_id)) from a left join b on 
